...
export function getNavRoute(index : number, params : {[key : string] : string | number} | null = null) : string{
    const nowTime = Date.parse(new Date().toString())
    let url = `${<string>staticRoutes[0].children?.[index - 1]?.path}?ts=${nowTime}`
    if(params != null){
...
export function setTitleFromRoute() {
    ...
    nextTick(() => {
        ...
        const value = `${webTitle}${siteConfig.siteName ? ' - ' + siteConfig.siteName : ''}`
        title.value = value
    })
}
 ...
 * 根据路由 meta.title 设置浏览器标题
 */
export function setDescriptionsFromRoute() {
    if (typeof router.currentRoute.value.meta.title != 'string') {
        return
    }
    nextTick(() => {
        let webTitle = ''
        if ((router.currentRoute.value.meta.title as string).indexOf('pagesTitle.') === -1) {
            webTitle = router.currentRoute.value.meta.description as string
        } else {
            webTitle = i18n.global.t(router.currentRoute.value.meta.description as string)
        }
        const title = useTitle()
        const siteConfig = useSiteConfig()
...
const AssetsUrl = "/storage/frontend/"
export const getAssetsUrl = (res: string) => {
    return fullUrl(AssetsUrl + res)
}
